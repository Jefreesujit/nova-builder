...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (text: string, attachments: Attachment[] = []) => {
    ...
    try {
      // 2. Call Gemini Service (Streaming)
      ...
      // Initialize a placeholder message for the bot
      ...
      for await (const update of generator) {
        ...
        if (update.summary) {
          ...
          // Update the last message (bot message) with current summary
          setMessages(prev => {
            const newMsgs = [...prev];
            const lastMsg = newMsgs[newMsgs.length - 1];
            if (lastMsg.role === 'model') {
              lastMsg.content = finalSummary;
            }
            return newMsgs;
          });
        }
      ...
      // 3. Final Updates
      // Update Project Name if provided and it's the first significant update
      if (finalProjectName && currentProjectId) {
        setProjects(prev => prev.map(p => {
          if (p.id === currentProjectId) {
            return { ...p, name: finalProjectName };
          }
          return p;
        }));
      }
      ...
      const errorMessage = error?.message || "Sorry, I encountered an error generating the code. Please try again.";
      ...
      setMessages(prev => {
        ...
        if (newMsgs.length > 0) {
          const lastMsg = newMsgs[newMsgs.length - 1];
          if (lastMsg.role === 'model' && lastMsg.content === "Generating...") {
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden font-sans transition-colors duration-200 ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'
      }`}>
      <Header
        user={user}
